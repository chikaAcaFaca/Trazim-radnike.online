...
export default function LoginPage() {
  ...
  // Check available OAuth providers on mount
  useEffect(() => {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || `${provider} prijava nije dostupna`);
      }

      // Redirect to OAuth provider
      window.location.href = data.data.url;
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Greška pri OAuth prijavi';
      setLocalError(message);
            ...
            <>
              <div className="space-y-3">
                {availableProviders.includes('google') && (
                  ...
                  >
                    <GoogleIcon />
                    {oauthLoading === 'google' ? 'Preusmeravanje...' : 'Nastavi sa Google'}
                  </Button>
                )}
                ...
                {availableProviders.includes('facebook') && (
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full flex items-center justify-center gap-3 text-[#1877F2] hover:text-[#1877F2] hover:bg-blue-50"
                    onClick={() => handleOAuthLogin('facebook')}
                    disabled={isLoading || oauthLoading !== null}
                  >
                    <FacebookIcon />
                    {oauthLoading === 'facebook' ? 'Preusmeravanje...' : 'Nastavi sa Facebook'}
                  </Button>
                )}
              </div>
